      ...
      <div className="px-5 pb-12 lg:px-0 lg:pb-0 lg:pt-8">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 text-center">
            <h2 className="text-base font-bold text-gray-900">{profile.title}</h2>
            <p className="mt-1 text-sm text-rose-600">{profile.subtitle}</p>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <a href={`mailto:${profile.email}`} className="flex items-center gap-2 rounded-md border border-gray-200 p-2 text-xs text-gray-700 hover:bg-rose-50 hover:border-rose-200">
              <Mail size={13} className="text-rose-500" />
              <span className="truncate">{profile.email}</span>
            </a>
            <a href={`tel:${profile.phone.replace(/\s/g, "")}`} className="flex items-center gap-2 rounded-md border border-gray-200 p-2 text-xs text-gray-700 hover:bg-rose-50 hover:border-rose-200">
              <Phone size={13} className="text-rose-500" />
              <span>{profile.phone}</span>
            </a>
            <div className="flex items-center gap-2 rounded-md border border-gray-200 p-2 text-xs text-gray-700">
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {languages.map((lang) => (
                <div key={lang.name} className="flex items-center justify-between rounded bg-gray-50 px-2 py-1.5 text-[11px]">
                  <span className="font-medium text-gray-700">{lang.name}</span>
                  <span className="text-[10px] text-gray-500">{lang.detail || lang.level}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-1">
              {softSkills.map((s) => (
                <div key={s.label} className="flex items-center gap-1 rounded-full bg-rose-50 px-2 py-0.5 text-[10px] text-rose-600">
            ...
            <div className="flex gap-2">
              {socialLinks.map((s) => (
                <a
                  ...
                  className="flex flex-1 items-center justify-center gap-1 rounded-md border border-gray-200 py-1.5 text-[10px] font-medium text-gray-700 hover:bg-gray-900 hover:text-white"
